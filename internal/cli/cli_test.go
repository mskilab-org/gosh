...
func TestExecuteWritesCommandHelp(t *testing.T) {
	tests := []struct {
		...
		{name: "status long help", args: []string{"status", "--help"}, wantUsage: "gosh status [--run-dir DIR] [--json]"},
		{name: "tasks short help", args: []string{"tasks", "-h"}, wantUsage: "gosh tasks [--run-dir DIR] [filters] [--json]"},
		{name: "inspect long help", args: []string{"inspect", "--help"}, wantUsage: "gosh inspect <task> [--run-dir DIR] [--json]"},
		{name: "index long help", args: []string{"index", "--help"}, wantUsage: "gosh index [--run-dir DIR] [--refresh] [--json]"},
	}
...
func TestRunStatusLogOnlyWithoutParseableFailuresStillRendersDegradedSummary(t *testing.T) {
	runDir := t.TempDir()
	logPath := filepath.Join(runDir, ".nextflow.log")
	logContent := strings.Join([]string{
		"Apr-28 12:00:00.000 [main] INFO nextflow.Session - Session start",
	...
	got := output.String()
	for _, want := range []string{"mode: log-only", "failed_count: 0", "log_only_no_parseable_failures", "Run future Nextflow workflows with -with-trace"} {
		if !strings.Contains(got, want) {
			t.Fatalf("RunStatus(log-only no failures) output = %q, want it to contain %q", got, want)
		}
	}
	assertNoIndexCacheDir(t, runDir)
}
...
func TestRunStatusUnsupportedRendersDiagnosticsAndReturnsCommandError(t *testing.T) {
	...
	got := output.String()
	for _, want := range []string{"unsupported_artifacts", "No supported Nextflow trace or log artifacts found", "Run future Nextflow workflows with -with-trace"} {
		if !strings.Contains(got, want) {
...
func TestRunTasksLogOnlyAndUnsupportedStateTasksUnavailable(t *testing.T) {
	t.Run("log-only", func(t *testing.T) {
		...
		got := output.String()
		for _, want := range []string{"tasks_unavailable_log_only", "complete task/resource/status data is unavailable", "Run future Nextflow workflows with -with-trace", logPath} {
			if !strings.Contains(got, want) {
...
func TestRunInspectLogOnlyAndUnsupportedDoNotFabricateTasks(t *testing.T) {
	t.Run("log-only", func(t *testing.T) {
		runDir := t.TempDir()
		logPath := filepath.Join(runDir, ".nextflow.log")
		logContent := strings.Join([]string{
		...
		if err == nil {
			t.Fatalf("RunInspect(log-only) returned nil error")
		}
		if !strings.Contains(err.Error(), "log-only") {
			t.Fatalf("RunInspect(log-only) error = %q, want it to mention log-only", err.Error())
		}

		got := output.String()
		for _, want := range []string{"inspect_unavailable_log_only", "complete task/resource/status data is unavailable", "Run future Nextflow workflows with -with-trace", logPath} {
			if !strings.Contains(got, want) {
				t.Fatalf("RunInspect(log-only) output = %q, want it to contain %q", got, want)
			}
		}
		for _, notWant := range []string{"selector:", "resolution:", "command_files:", "PIPE:ALIGN"} {
			if strings.Contains(got, notWant) {
				t.Fatalf("RunInspect(log-only) output = %q, did not want fabricated inspect marker %q", got, notWant)
			}
		}
		assertNoIndexCacheDir(t, runDir)
	})
...
func TestRunIndexUnsupportedWithoutRefreshReportsDiagnosticsWithoutCreatingCache(t *testing.T) {
	...
	got := output.String()
	for _, want := range []string{"unsupported_artifacts", "Run future Nextflow workflows with -with-trace"} {
		if !strings.Contains(got, want) {
